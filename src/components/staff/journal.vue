      ...
      <div class="tw-flex-row tw-flex tw-w-full tw-flex-wrap sm:tw-flex-nowrap">
        ...
        >
          <div class="tw-flex tw-w-full">
            <div
              ...
              :class="{ focus: isFocusing || search1 }"
            >
              <input
                :placeholder="$t(`searchFromName`)"
                ref="input"
                v-model="search1"
                v-bind="{ ...$props, ...$attrs }"
        ...
        >
          <div class="tw-flex tw-w-full">
            <div
              class="input-wrapper tw-h-[48px] tw-flex align-center tw-border tw-border-light-grey tw-bg-input-bg tw-w-full"
              :class="{ focus: isFocusing1 || search2 }"
            >
              <input
                :placeholder="$t(`searchFromName`)"
                ref="input1"
                v-model="search2"
                v-bind="{ ...$props, ...$attrs }"
                @focus="isFocusing1 = true"
                @blur="isFocusing1 = false"
                class="tw-w-full tw-h-full py-3 px-4 border-none no-outline"
...
export default {
  ...
  data() {
    return {
      ...
      tabs: [
        {
          id: 1,
          text: "communicationArtsAndManagementInnovation",
          slug: "communication-arts-and-innovation",
        },
        {
          id: 2,
          text: "communicationAndManagement",
          slug: "communication-and-management",
        },
      ],
      search1: "",
      search2: "",
  ...
  created() {
    ...
    this.selectedtab = this.$route.path.includes(
  ...
  methods: {
    ...
    handleSearch1() {
      if (!this.search1) {
        ...
        let filterTH = this.itemsAll1.filter(
          (item) =>
    ...
    handleSearch2() {
      if (!this.search2) {
        ...
        let filterTH = this.itemsAll2.filter(
          (item) =>
    ...
    async getAll() {
      ...
      if (this.$store.state.lang == "th") {
        const response = await this.axios.get(
          ...
          }/academicJournals?type_th=นิเทศศาสตร์และนวัตกรรม&offset=${
            this.offset
          }&limit=${this.limit}${
        ...
        const response2 = await this.axios.get(
          `${
            process.env.VUE_APP_API
          }/academicJournals?type_th=การสื่อสารและการจัดการ&offset=${
            this.offset2
          }&limit=${this.limit2}${
    ...
    gotohowto(id) {
      this.$router.push(`/journal-howto`);
    },
    gotodetail(item) {
      const pathParam = this.tabs.find(
        (obj) => obj.id == this.selectedtab
      ).slug;
      this.$router.push(`/journal/${pathParam}/${item.id}`);
    },
    handleScroll() {
